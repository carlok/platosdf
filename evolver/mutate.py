...
# ── Resonant vocabulary ───────────────────────────────────────────────────────
# FD corners and edge midpoints in barycentric (u,v) — same for all groups.
# Slightly inset from exact corners so fd_point() doesn't degenerate.
#
#   c0 = (0,0) → 3-fold axis  (Platonic vertices)  ← orbit size: |G|/|stab_C3|
#   c1 = (1,0) → 2/4-fold axis (edge midpoints)    ← orbit size: |G|/|stab_C2|
#   c2 = (0,1) → highest-fold axis (face centres)  ← orbit size: |G|/|stab_C4|
#
_RESONANT_FD = [
    (0.04, 0.04),   # near c0 — vertex axis
    (0.88, 0.04),   # near c1 — edge midpoint axis
    (0.04, 0.88),   # near c2 — face centre axis
    (0.46, 0.04),   # midpoint of c0-c1 FD edge
    (0.04, 0.46),   # midpoint of c0-c2 FD edge
    (0.44, 0.44),   # midpoint of c1-c2 FD edge
]

# Natural radii: circumradius / midradius / inradius of the seed solid.
# Placing a primitive at these distances aligns with the group's own metric.
_RESONANT_DISTANCES = {
    "tetrahedral":  [1.00, 0.82, 0.58],   # R, √(2/3), 1/√3
    "octahedral":   [1.00, 0.71, 0.58],   # R, 1/√2,   1/√3
    "icosahedral":  [1.00, 0.85, 0.58],   # R, φ/√(1+φ²), 1/√3
}
...
def resonant_grammar_pure(
    n_steps: int | None = None,
    seed_type: str | None = None,
) -> dict:
    """
    G-grammar using only group-theoretically natural parameter values:
    - fd position: one of the 6 FD corners / edge midpoints
    - distance: one of the 3 canonical radii (circum/mid/in) of the seed solid
    - scale_factor: continuous (not constrained — orbit size handles density)
    - smooth_radius: continuous
    Bypasses _FD_EPS inset — corners are the point, not a problem here.
    """
    if n_steps is None:
        n_steps = random.randint(1, 4)
    seed_prim = seed_type or random.choice(PRIMITIVES)
    group = SEED_TO_GROUP[seed_prim]
    radii = _RESONANT_DISTANCES[group]

    steps = []
    for _ in range(n_steps):
        u, v = random.choice(_RESONANT_FD)
        steps.append({
            "operation":     random.choice(OPERATIONS),
            "primitive":     random.choice(CHILD_PRIMITIVES),
            "fd_u":          round(u, 4),
            "fd_v":          round(v, 4),
            "distance":      random.choice(radii),
            "scale_factor":  round(random.uniform(0.15, 0.55), 3),
            "smooth_radius": round(random.uniform(0.0, 0.06), 4),
        })
...
def resonant_population(pop_size: int) -> list[dict]:
    """Equal thirds, all resonant grammars."""
    population = []
    per_type = pop_size // len(PRIMITIVES)
    for prim in PRIMITIVES:
        for _ in range(per_type):
            population.append(resonant_grammar_pure(
                n_steps=random.randint(1, 4), seed_type=prim
            ))
    while len(population) < pop_size:
        population.append(resonant_grammar_pure())
