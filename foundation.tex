...
\begin{itemize}
  ...
  \item $b_i \in \{\mathrm{subtract}, \mathrm{intersect}\}$ is a Boolean operation
        (union excluded: orbit copies placed near the seed surface protrude as
        peninsulas, which are unprintable and topologically undesirable),
...
Since $b_i \in \{\mathrm{subtract}, \mathrm{intersect}\}$, the grammar is
\emph{carving-only}: every operation reduces the solid, ensuring the
resulting shape is a connected subset of the seed primitive.
...
\paragraph{Mutation.}
With rate $p_m = 0.65$, each gene mutates independently:
\begin{itemize}
  \item $(u_i, v_i) \leftarrow \mathrm{clamp}((u_i + \mathcal{N}(0, 0.22),\; v_i + \mathcal{N}(0, 0.22)),\; [0,1]^2)$, then renormalise if $u_i + v_i > 1$.
  \item $b_i$ resampled from $\{\mathrm{subtract, intersect}\}$ uniformly.
